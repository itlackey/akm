 ...
 * `src/core/adapter/adapters/` — akm 0.9.0 chunk-2, WI-2.1.
 *
 * ── Cycle-safety (chunk-2 brief's "cycle-safety watch," finding 8) ──
 *
 ...
 * ported here — it is memory/session-specific and none of WI-2.1's three
 * fixtures (skill/wiki/script) use it. WI-2.4 (memory/note adapters) should
 * extend `extractRefTokens`'s caller with that carve-out when it ports
 * memory/session.
 *
 ...
 * module needs the strip for. Not exercised by any WI-2.1 golden fixture
 * (none contain fenced refs).
 */
...
// ── Small pure helpers, reused across skill/wiki/script (and future adapters) ──
...
// ── Base validate checks (port of `BaseLinter.runBaseChecks`, base-linter.ts:583-741) ──
...
// transitively into `core/adapter/`'s graph). `KNOWN_TYPES` covers all 14
// built-in types; a custom `registerAssetType`-registered extension type
// would not be recognized here — an accepted, flagged simplification (no
// WI-2.1 fixture uses a custom type).
...
/**
 * Port of `BaseLinter.runBaseChecks` (`commands/lint/base-linter.ts:583-741`),
 * adapted to the read-only `ValidateContext`/`Diagnostic` shapes (adapter
 * spec §12.1; D2-3). Reproduces unquoted-colon / missing-updated /
 * stale-path / missing-ref for ONE file's already-parsed content.
 *
 * `fixed` is always `false`: `BundleAdapter.validate` MUST NOT write the
 * filesystem (`bundle-adapter.ts:96-97`), so there is no fix-apply
 * mechanism at this layer — a documented, intentional behavior narrowing
 * from the legacy `--fix`-capable linter (flagged in the WI-2.1 report; does
 * not affect byte-for-byte parity against the lint golden, whose fixtures
 * are lint-clean and carry no `fixed: true` entries to reproduce).
 *
 ...
 * parse frontmatter and run {@link runBaseValidateChecks}. This is the
 * ENTIRE `validate()` body for adapters with no per-type extra checks
 * (wiki/script — DefaultLinter-equivalent, D2-3); adapters with extra checks
 * (skill's missing-skill-md) call this first and append their own pass.
 */
