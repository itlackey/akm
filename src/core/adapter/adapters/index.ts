 ...
 * Exercised today only by this WI's own tests
 * (`tests/core/adapter/registry.test.ts`).
 *
 * Later WIs (2.2 workflow/task, 2.3 dotenv, 2.4 the markdown family, 2.5
 * skill's §4.5 contract) extend `BUILTIN_ADAPTERS`/`registerBuiltinAdapters`
 * the same way this WI extended chunk-1's empty `src/core/adapter/`: add the
 * adapter's own file under this directory, import it here, add one
 * `registerAdapter(...)` call (with an explicit `types` array for any
 * multi-type adapter — see `registry.ts`'s header for why).
 */
...
export { scriptAdapter, skillAdapter, wikiAdapter };

/** The adapters this WI mints, in a stable order. */
export const BUILTIN_ADAPTERS = [skillAdapter, wikiAdapter, scriptAdapter] as const;
