 ...
 *
 * Retry strategy: if the lock is held we yield via `Bun.sleepSync` (10 ms)
 * when available, otherwise fall back to a busy-wait counter.  After a 5 s
 * deadline we throw rather than silently proceeding — a timeout here is always
 * a programming error or a stale lock left by a crashed process.
 */
function withVaultLock<T>(vaultPath: string, fn: () => T): T {
  ...
  while (true) {
    try {
      ...
      // Prefer Bun.sleepSync when available to avoid pegging the CPU.
      if (
...
export function unsetKey(vaultPath: string, key: string): boolean {
  ...
  return withVaultLock(vaultPath, () => {
    ...
    // Walk backwards from the key line to find any run of consecutive comment
    // lines (matching /^\s*#/) that immediately precede it.  A blank line or
    // assignment between a comment and the key breaks the association — we stop
    ...
    for (let i = keyLineIdx - 1; i >= 0; i--) {
      if (/^\s*#/.test(lines[i])) {
        ...
        break; // blank line or assignment — stop
      }
    }
