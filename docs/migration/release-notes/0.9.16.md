...
**`--skip-if-locked` keeps its purpose, by a different mechanism.** Index runs
no longer take a rebuild lock at all, so there is no longer a lock to test
before starting. The flag now decides how a genuine collision is REPORTED: a
concurrent run that cannot get its write through, and would otherwise fail
with exit 75, instead skips gracefully with
`{ ok: true, skipped: { reason: "contended", code } }` and exit 0. A scheduled
or hook-driven `akm index` therefore still never fails on a double launch,
which is what the flag was always for. Without the flag, the transient error
still surfaces at exit 75 so an interactive run is told the truth. The
`skipped.reason` changes from `"lock-held"` to `"contended"`, and the
`pid`/`launcherPid`/`startedAt` fields are gone with the lock that supplied
them.
