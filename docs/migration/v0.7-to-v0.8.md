...
## Config v2 migration (reflect multi-mode)

0.8.0 ships a new config shape (`configVersion: "0.8.0"`) that reorganizes LLM
and agent configuration into a unified `profiles` + `features` tree. This
enables reflect to run as a direct LLM call (3–5× faster) and supports multiple
named LLM connections without copy-pasting endpoint/apiKey across feature flags.

### What changed

Three top-level sections replace the scattered v1 keys:

- **`profiles.llm.<name>`** — each named LLM endpoint declared once (endpoint,
  model, apiKey, temperature, supportsJsonSchema).
- **`profiles.agent.<name>`** — each named agent runtime, now with a required
  `platform: "opencode" | "claude" | "opencode-sdk"` field.
- **`defaults.llm` / `defaults.agent`** — fallback profile names.
- **`features.improve.*` / `features.index.*` / `features.search.*`** — every
  named LLM/agent operation in the app, with a unified
  `{mode, profile, timeoutMs, options}` shape replacing both the old
  `llm.features.*` boolean flags and the old `agent.processes` map.
...
| --- | --- |
| `config.llm` (endpoint/model/apiKey/temperature) | `profiles.llm.<name>` |
| `config.llm.judgeModel` | `profiles.llm."openai-judge".model` (separate profile) |
| `config.agent.profiles.<n>` | `profiles.agent.<n>` |
| `config.agent.default` | `defaults.agent` |
| `config.agent.processes.<name>` | `features.<section>.<name>` (see below) |
| `config.improve.reflectCooldownByType` | `features.improve.reflect.options.cooldown` |
| `config.improve.limit` | `defaults.improve.limit` |
| `config.llm.features.memory_inference` | `features.index.memory_inference` |
| `config.llm.features.graph_extraction` | `features.index.graph_extraction` |
| `config.llm.features.metadata_enhance` | `features.index.metadata_enhance` |
| `config.llm.features.memory_consolidation` | `features.improve.memory_consolidation` |
| `config.llm.features.feedback_distillation` | `features.improve.feedback_distillation` |
| `config.llm.features.curate_rerank` | `features.search.curate_rerank` |
...
# Confirm features tree populated
akm config get features.improve.reflect
