...
0.8.0 is a storage reorganization release. The CLI surface introduced in 0.7.0
(proposal queue, `akm reflect`, `akm distill`, `akm propose`, the `lesson`
asset type, `llm.features.*` gates) is unchanged. What changes is where akm
stores its runtime data and which legacy write paths survive.
...
| `reflect_completed` | `akm reflect` (after proposal created) | Links invocation to proposal ID |
