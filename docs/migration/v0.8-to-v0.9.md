...
- **`env`** — an entire `.env` file, sourced or injected **wholesale**. akm no
  longer edits entries; you edit the file with your own editor and akm loads it.
- **`secret`** — a single opaque value (one file = one value), for the cases
  where `vault set <ref> <KEY>` was used to store one credential.

Key names and comments are still surfaced for discoverability; values are still
never written to stdout, the index, or any structured output.
