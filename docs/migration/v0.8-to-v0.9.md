...
deprecations (the CLI aliases and the `vault` asset type). The durable-state
re-key, database merge, and config migration are handled by the journaled,
crash-resumable `akm migrate apply` coordinator; the manual boundary procedure
below is how you cross from a 0.8 binary to a 0.9 one safely.
It does not translate profile-based configuration, workflow, or task files
automatically. Create the recovery backup before changing a live installation,
then migrate the affected assets deliberately.

## The 0.9.0 bundle-adapter refactor

- **Ref grammar: `type:name` → `[bundle//]conceptId`.** Refs are now
  subdir-qualified concept ids inside their bundle — `skills/code-review`,
  `memories/vpn-note`, `knowledge/api-guide`, `env/prod`, `secrets/deploy-token`
  — with an optional `bundle//` installation prefix and an optional `#fragment`.
  Durable state stores the fully-qualified `bundle//conceptId`; the short
  bundle-omitted form is accepted input only (CLI, API, and inside bundle
  content), resolved against `defaultBundle` and then installation-priority
  order. The pre-0.9.0 `[origin//]type:name` grammar is removed — there is no
  compatibility parser. `akm migrate apply` re-keys every durable ref
  (usage/feedback events, proposal targets, workflow/task targets, salience) to
  the new spelling; refs embedded in your own asset bodies are rewritten by the
  content migration, and unresolvable refs are quarantined, not dropped.
- **Databases: four → three.** The former `workflow.db` is folded into
  `state.db`; the three databases are `state.db` (durable workspace state),
  `index.db` (the fully regenerable search cache), and a separate `logs.db`.
- **Config: flat keys → `bundles` / `defaultBundle`.** `stashDir`, `sources`,
  `installed`, and `wikiName` are migrated into a `bundles` map keyed by each
  source's stable id, with `defaultBundle` naming the primary writable bundle.
  Bundle ids are derived from the existing `registryId` / path slug, so no
  second identity migration happens. After the cutover the retired keys are
  **hard-rejected** by the 0.9.0 schema whenever present — a config still
  carrying them fails to load with an error naming `akm migrate apply`.
  Registry-installed bundles keep only their desired locator (`git`/`npm` +
  `registryId`) in config; resolved cache paths and revisions live exclusively
  in the lockfile.
- **`index.md` / `log.md` are reserved.** Per the Open Knowledge Format, these
  are directory/structure files at every bundle depth — never indexed as
...
- **Update agent instructions.** Any prompt, `AGENTS.md`, or doc that still
  spells refs in the old `type:name` form must switch to the subdir-qualified
  form (e.g. a code-review skill is now `skills/code-review`). See `STABILITY.md`
...
- [What the `vault` split became](#what-the-vault-split-became)
- [Running the migration](#running-the-migration)
- [Command mapping](#command-mapping)
- [The safe load paths](#the-safe-load-paths)
...
The `vault` type was split into `env` and `secret` during the 0.8 line; 0.9.0
removes the `vault` type entirely and re-spells refs as subdir-qualified concept
ids. The mapping (right column is the current 0.9.0 world):
...
- The `vault:` ref alias (parsing a `vault:` ref becomes an unknown-type error
  pointing at the `env/` and `secrets/` concept ids).
