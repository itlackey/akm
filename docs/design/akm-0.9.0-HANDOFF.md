...
- **Chunk 0a** (hygiene goldens) — merged, BUT ⚠️ many golden test files were later deleted during a test-suite cleanup (commit `3927ff9`). The behavior baselines the plan assumed for Chunks 6/7/9 are **partially gone** — re-capture what you need before refactoring.
