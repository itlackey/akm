# akm 0.9.0 Execution Workflow — Creation Record

**Date:** 2026-07-14. **Author:** Claude (Fable 5), supervising session for the 0.9.0 refactor.
**Companions:** `akm-0.9.0-execution-workflow.md` (runbook), `akm-0.9.0-chunk-manifest.json` (derived index), `.claude/workflows/akm-090-chunk.js` (the machinery — a verbatim copy is embedded in §7 below).
**Commit trail:** `b888177` (initial machinery), `b92811e` (hardening from adversarial verification), `c19948b` (integration-branch + usage-gate adjustments).

This document records how the workflow was created — including what the verification pass caught and what the usage-gate spike proved — and explains how the machinery works, step by step.

---

## 1. The mandate

The maintainer's process specification, which the workflow implements exactly:

> design a workflow around this plan that will complete all of the 0.9.0 work using teams of agents executing test first and review gated development processes to implement the tasks. fable 5 ultracode should be used to create the implementation details for the task(s) to be handed to the implementation team. these details should be grounded in the plan and the codebase before handing to the team. this is the most critical step and ensures each teams work aligns with the overall plan and produces exactly the code needed to complete that portion of the plan. sonnet 5 should be used as the developer agent with opus 4.8 used for the review agents. reviews should measure against know code quality metrics as well as strict adherence to the implementation details. if the review fails twice, opus should escalate to fable 5 for assitance and clarification. if the review still fails, mark those tasks as blocked and escalate to a human for review and guidance.

Two later adjustments: (a) all work bases from and merges back into `claude/akm-architecture-refactor-fubvd7` — no separate integration branch; (b) a Sonnet agent checks the account's Claude Code usage windows (current 5-hour + others) and pauses the workflow until credits are available, with a spike proving the calculation works.

The plan (`akm-0.9.0-bundle-adapter-architecture-plan.md`) is THE authority throughout. The workflow never lets an agent treat current rc-train code as normative except where the plan explicitly says "behavior preserved".

## 2. How it was built

### 2.1 Grounding before authoring

Before writing a line of the workflow, the ground truth was re-established:

- **Plan §11** (the two-wave chunk order and each chunk's paragraph), **§12** (ledger, DoD, contract tests, risks), and **§15** (test strategy rules 1–9) were re-read at HEAD.
- The repo's **real commands** were read from `package.json` (`bun run check`, `check:fast`, `lint`, `test:integration`, `gen-config-schema --check`) so agent prompts name commands that actually exist — version at the time: `0.9.0-rc.5`.
- The Workflow runtime's constraints were collected: scripts are plain JS with a pure-literal `meta` export; the body runs in an async wrapper (top-level `await`/`return` are legal); available globals are only `agent() / parallel() / pipeline() / phase() / log() / args / budget / workflow()`; `Date.now()`, `Math.random()`, and argless `new Date()` throw (they would break resume); `agent()` returns `null` when a subagent dies; `parallel()` thunks that throw resolve to `null`; structured output is enforced by passing a JSON Schema per `agent()` call.

### 2.2 The manifest: plan §11 as data

The workflow needed the plan in machine-readable form. `akm-0.9.0-chunk-manifest.json` was derived from §11: all 14 chunks with id, wave, execution order, branch name, faithful scope condensation, per-chunk gates, net-LOC estimate, §15 test-bucket mapping, and plan references — plus the global gates (zero-count greps, safety suites, hard rules). Its header declares it a *derived index*: on any conflict the plan wins, and §11 changes require regeneration.

### 2.3 The script and the runbook

`.claude/workflows/akm-090-chunk.js` was authored against those constraints (architecture in §3–§6 below), and `akm-0.9.0-execution-workflow.md` documents the process for humans. Syntax was validated by replicating the runtime's wrapping — prepending declarations of the injected globals and enclosing the body in an `async function` — and running `node --check` on the result (a bare `node --check` false-fails on the legal top-level `return`).

### 2.4 Adversarial verification (and what it caught)

A six-agent ultracode verification pass reviewed the whole package before first use: three Fable 5 agents checked manifest fidelity against plan §11 chunk-by-chunk; an Opus 4.8 agent audited the script's logic against the runtime contract; an Opus 4.8 agent checked runbook-vs-script coherence; a Fable 5 agent checked the package against the maintainer's specification verbatim. Result: **0 blockers, 11 majors, 13 minors — all fixed** in `b92811e`. The catches worth recording:

1. **Manifest fold-in omissions.** Plan §13.1/§14.2/§14.3 assign work to Chunks 7 and 9 *outside* their §11 paragraphs (the ~1,300-LOC recombine subsystem deletion + orphaned `db.ts` exports to Chunk 7; the echarts CDN flip, the deterministic-embedder relocation with its `AKM_EMBED_DETERMINISTIC`-stays-in-the-facade trap, rank-metrics move, html-render trim, H1/H2/H3/L1, and resume-field deletion to Chunk 9). A manifest-driven run would have silently skipped all of it. The entries now carry the fold-ins explicitly.
2. **Fail-open brief verification.** A dead verifier lens contributed zero blockers, so three dead lenses would have *approved* the brief unverified — inverting the criticality of the one step everything else depends on. Now: dead lenses retry once, then force revision; `revise` verdicts with empty blocker lists and incomplete rounds also force revision; approval requires a complete round.
3. **Trusted self-reported verdicts.** The review gate originally read the reviewer's `pass` boolean; a reviewer returning `pass=true` alongside a blocker finding would have gated the item through and the blocker discarded. Verdicts are now derived: `pass === true AND zero blocker/major findings`, with a logged warning on disagreement.
4. **Infra noise consuming ladder rungs.** A transient developer-agent death was recorded as a failed attempt, so two crashes plus one real failure could trigger Fable escalation — or mark an item blocked — on infrastructure noise. Dev calls now retry once on death, mirroring the review path.
5. **Unreviewed repair commits.** The chunk-gate repair pass let a Sonnet dev commit code validated only by re-running gates — a hole in "review-gated development". Repair diffs now get the same dual Opus review; rejected repairs are flagged for the human, never silently kept.
6. **Grep gates ahead of their chunks.** The ten zero-count greps are end-state gates that only become satisfiable at specific chunks (`StashEntry` at 5, `TYPE_DIRS` at 3, …). Applied at every boundary they would have fed the repair dev impossible "failures" — e.g. instructions to delete identifiers a later chunk owns. Each grep is now annotated with its effective chunk and gate runners skip greps whose chunk hasn't landed.
7. **A latent plan defect.** Plan §3.4's frozen legacy-resolver copy (`src/migrate/legacy/`) deliberately retains dead identifiers — which sat inside the §11 grep-gate scope, guaranteeing DoD failure or a silent identifier rename that would defeat the copy's frozenness. Plan §11 was amended: the grep scope now excludes `src/migrate/legacy/`.
8. **Runbook overclaims.** "Pushed whatever the outcome" was false for the three pre-Finalize aborts; the human-gate instructions routed readers to a report that wouldn't exist. Corrected, with the diagram's missing `recommendBlock` early-block edge added.

### 2.5 The usage-gate spike

The maintainer required a usage gate and a spike proving the measurement works. Every surface available in the remote container was tested:

- **OAuth `/api/oauth/usage`** (what the `/usage` screen uses): unreachable — `~/.claude/.credentials.json` holds only MCP OAuth material in remote containers; no Claude Code access token exists on disk (and the permission classifier rightly blocks credential materialization).
- **Session transcripts** (`~/.claude/projects/**/*.jsonl`): carry per-request token usage but no account-level windows; they also can't see other sessions on the account. Not authoritative. (An early grep "hit" for rate-limit fields turned out to be self-referential — the search command's own text in the transcript.)
- **The winner:** a child `claude -p` call *does* authenticate in the container, and running it with `ANTHROPIC_LOG=debug` exposes the **unified rate-limit headers** on the API response — `anthropic-ratelimit-unified-{5h,7d}-{status,utilization,reset}` plus `unified-status`, `representative-claim`, and `overage-status`. These are the authoritative account-level windows the API actually enforces, obtainable for the cost of a ~10-token Haiku probe. The SDK debug log redacts the auth header (`authorization: "***"`).

**Proof:** a Sonnet 5 agent was given the gate procedure and independently produced: 5h window 40.0% used / 60.0% headroom, reset epoch `1784021400` → `2026-07-14T09:30:00Z`; 7d window 24.0% used, reset `1784563200` → `2026-07-20T16:00:00Z`; correct verdict and wait computation. Every value matched ground truth measured independently minutes earlier — including the trap that `utilization` is a **fraction** (0.4 = 40%), not a percent.

**Design consequence:** the probe showed `overage-status: rejected` — at 100% utilization *every* request fails, including the sleeper agent that implements the pause. The gate therefore trips early (90% of the 5h window, 97% of 7d) while pausing is still affordable.

### 2.6 Dispatch lesson

On first dispatch, the runtime did not forward the `args` object to the script (by `name` or by `scriptPath`), tripping the `args.chunk` guard. Workaround, now the standard dispatch pattern: generate a per-chunk **launch copy** of the canonical script with the chunk id defaulted in —
`sed "s|const chunkId = args && args.chunk$|const chunkId = (args \&\& args.chunk) || '<id>'|" .claude/workflows/akm-090-chunk.js > launch-chunk-<id>.js` — and invoke `Workflow({scriptPath: <launch copy>})`. The canonical script stays arg-driven for runtimes where args forwarding works.

## 3. How it works — the shape

One workflow invocation executes **one plan chunk** end-to-end. Chunks run sequentially in manifest order (Wave 1: 0a → 7 → 6 → 9; Wave 2: 0b → 1 → 1.5 → 2 → 3 → 4 → 5 → 6.5 → 8 → 10), each basing on the integration branch that contains its predecessors. The supervising session sits between chunks: it reviews each result, merges `akm-090/chunk-<id>` into `claude/akm-architecture-refactor-fubvd7`, and dispatches the next.

Phases, in execution order:

| Phase | Model | What happens |
|---|---|---|
| **Load** | Haiku 4.5 | Reads the manifest entry for the chunk verbatim (schema-forced), plus global gates. |
| **Usage Gate** | Sonnet 5 | Probes the unified rate-limit headers; proceeds, pauses in-run, or exits `paused-usage` (§5). Re-runs before every work item and before Finalize. |
| **Setup** | Sonnet 5 | Creates (or resumes) a dedicated git worktree at `/home/user/akm-worktrees/chunk-<id>` on branch `akm-090/chunk-<id>` off the integration branch; `bun install`; runs `check:fast` — a red baseline aborts the run (`blocked-baseline`): nothing can be review-gated against a red baseline. |
| **Ground** | Fable 5 | The critical step (§4): plan extractor → parallel codebase grounders → brief author. |
| **Verify Brief** | Fable 5 | Adversarial 3-lens panel; up to 2 brief revisions; unanimous, complete-round approval required or the run exits `blocked-grounding`. |
| **Implement** | Sonnet 5 | One work item at a time, test-first, enforced by commit order. |
| **Review** | Opus 4.8 ×2 | Adherence + quality reviews in parallel; verdicts derived from findings. |
| **Escalate** | Fable 5 | After two review failures: diagnosis + guidance (may amend the brief, may recommend blocking). After three: committed human escalation report. |
| **Finalize** | Sonnet 5 / Opus 4.8 | Whole-chunk gate run → one review-gated repair pass if red → Opus audit + committed `report.md` → branch push (never a PR). |

## 4. The grounding pipeline (the critical step)

The maintainer's spec calls the implementation details "the most critical step", so it gets the most machinery:

1. **Plan extractor** (Fable 5, high effort): reads the chunk's §11 paragraph and every section its manifest entry cites; produces a complete requirements inventory — each requirement with a stable id, exact plan anchor, and kind — plus 3–8 grouped grounding tasks. Instructed to neither invent nor drop requirements; completeness is adversarially checked downstream.
2. **Codebase grounders** (Fable 5, parallel — a justified barrier, since the author needs all results): each verifies its requirements against the actual code at the chunk's base commit — re-measuring every file:line anchor at HEAD rather than trusting the plan's (plan §12.4 documents anchor drift), recording current behavior, existing tests, and any plan-vs-code drift. A grounder that dies twice surfaces its requirements as explicitly **UNGROUNDED** rather than silently vanishing.
3. **Brief author** (Fable 5, xhigh effort): synthesizes the implementation brief — ordered work items sized for one dev session each, with `testMode`, `testsFirst` (specific tests to write before implementation), file-anchored steps, exact deletions, checkable acceptance criteria traceable to requirement ids, and a gate checklist mapping every manifest gate to the items satisfying it. The brief is written to `docs/design/execution/chunk-<id>/brief.md` and committed — a durable, human-reviewable artifact.
4. **Adversarial verification** (Fable 5 ×3, distinct lenses, default stance REFUSE): *plan fidelity* (every requirement covered, nothing beyond plan scope — trust-shaped or lifecycle-shaped additions are blockers by plan §1.3), *code grounding* (spot-verifies anchors and behavior claims against the files), *test adequacy* (would `testsFirst` actually catch a wrong implementation; are testModes right; do deletions carry same-commit replacement tests per plan §15 rule 4). Blockers trigger an author revision (max 2); the gate **fails closed** — dead lenses, empty-blocker `revise` verdicts, and incomplete rounds all force revision, and a brief that can't pass a complete round exits the run as `blocked-grounding` rather than reaching a developer.

## 5. The development loop and the escalation ladder

Work items run **sequentially** (they interdepend; one shared worktree, never parallel writers). Per item:

- **Dev attempt** (Sonnet 5): reads the brief; follows the test-first protocol for its `testMode` — *test-first* (tests committed separately first, recorded failing for the right reason, then made green; weakening an assertion is a blocker), *characterization-preserve* (goldens must pass unchanged, byte-for-byte where deterministic), *deletion-gate* (zero-count grep is the test; replacement contract test lands in the same commit), *docs-assets* (lints verify). Scope discipline is explicit: exactly this item, nothing trust- or lifecycle-shaped ever, deviations recorded for reviewers.
- **Dual review** (Opus 4.8, parallel, read-only): the **adherence reviewer** treats the brief as the contract — diff evidence per step, commit-order proof of test-first, out-of-scope changes flagged, deviations judged; the **quality reviewer** measures complexity/function length, DRY, SRP/coupling/dependency direction, naming/idiom, type safety, dead code, error handling, test quality, and commit hygiene. Both re-run commands; neither trusts the dev report over the code. Pass = both reviewers pass **and** zero blocker/major findings (derived in code).
- **The ladder**: fail 1 → dev revises with the findings. Fail 2 → **Opus escalates to Fable 5**, which examines the actual code, diagnoses the root cause (dev misread vs brief defect vs reviewer error), issues step-by-step guidance, may amend the brief (its one permitted write), or recommends blocking outright if the item is mis-scoped against the plan. Fail 3 → the item is **blocked**: a Fable-written escalation report (`escalation-<item>.md`) is committed with the unresolved findings verbatim, the diagnosis, and the specific decisions the human must make; work-in-progress commits stay; dependent items are skipped; the rest of the chunk proceeds. Only review failures consume ladder rungs — agent deaths retry once first.
- **Bounded context**: only the latest review round is passed to the dev in full; earlier rounds compress to counts, so prompts don't grow unboundedly across attempts.

## 6. Finalize, statuses, and the usage gate

**Finalize** runs the whole-chunk gate suite (`bun run check`, the chunk's manifest gates, the global gates *in effect* for this chunk, safety suites, net-LOC actuals — reported, never gated, per plan §1.3), allows one repair pass whose diff gets the same dual Opus review, then an Opus auditor cross-checks everything and commits `report.md`. The branch is pushed (never a PR); for runs that abort before Finalize nothing is pushed and the structured result is the record.

**Statuses:** `complete` (all items done, gates green, audit passed — merge it) · `blocked` (≥1 item escalated to the human) · `partial` (items deferred on the session token budget) · `needs-human` (items passed but gates/audit failed) · `paused-usage` · `blocked-setup` / `blocked-baseline` / `blocked-grounding` (pre-Finalize aborts).

**Usage gate** (spike details in §2.5): before the chunk starts, before every work item, and before Finalize, a Sonnet agent runs the ~10-token probe, parses the `anthropic-ratelimit-unified-*` headers, converts fraction utilizations to percentages, and verdicts: proceed iff `unified-status=allowed` AND 5h ≤ 90% AND 7d ≤ 97% (args-tunable). On violation: waits within 6h (any 5h-window reset) pause **in-run** — a Sonnet sleeper sleeps in ≤10-minute slices to one minute past the reset epoch, then the gate re-probes rather than trusting the clock; longer waits (a 7-day window) exit as `paused-usage` with the reset time, best-effort pushing completed work first. A probe that fails twice is treated as possibly-already-exhausted and exits the same way rather than guessing. Gate agents extract only rate-limit fields from the probe log and delete it.

**Reliability principles applied throughout:** every single-agent step retries once on death; verification and review gates fail **closed** (a dead verifier can never approve); grounding failures fail **loud** (explicit UNGROUNDED markers); verdicts are derived from findings, not self-reported booleans; diagnostics are never destroyed by a failed re-run; and every prompt carries the plan's hard rules (§1.3) verbatim so no agent "helpfully" reintroduces trust machinery or the deferred memory lifecycle.

## 7. The workflow script (verbatim copy)

The canonical script at `.claude/workflows/akm-090-chunk.js`, exactly as dispatched (launch copies differ only in the `chunkId` default, per §2.6):
