...
- **Revision 2** (adversarial-panel corrections): §2.5 safety-suite set now covers all five plan-named categories incl. embedded symlink coverage (29-file boundary set, replayed in WI-08 step 3); the test-LOC baseline states both denominators — the plan §15 header figure (588/175,041, all `tests/**/*.ts`) is exact, not stale; WI-02's P0-A once-per-asset scenario now emits the `reflect_invoked` cursor event explicitly (the gate is event-driven — a stubbed `reflectFn` never emits it).
...
- Safety suites (§15.3, must be green at chunk boundary). Plan §15 rule 3 names **five** categories: traversal/escape, **symlink**, redaction/dangerous-key, SQLite journal/busy/lock/contention/cross-proc, migration. At this HEAD they map to:
  - **Dedicated safety files** (four of the five categories): 19 files / 4,240 LOC — unit scope: `env-traversal`, `workflow-path-escape`, `redaction`, `config-cli-redaction`, `env-run-dangerous-key-block`, `vault-dangerous-key-install-gate`, `vault-dangerous-key-lint`, `sqlite-journal-mode`, `db-busy-timeout`, `index-writer-lock`, `migration-lifecycle-regression`, `migration-backup`; integration scope: `tar-utils-scan`, `git-source-safety`, `index-writer-lock-crossproc`, `workflow-db-contention`, `file-lock`, `improve-lock-serialization`, `migration-apply-crash`.
  - **Symlink handling has NO dedicated suites at this HEAD** — it lives embedded as cases inside 11 broader files (grep-verified, 5,777 LOC total), of which only `tests/workflow-path-escape.test.ts` overlaps the dedicated set. The 10 additional symlink-coverage carriers (5,604 LOC) — unit scope: `tests/commands/improve/improve-dry-run-side-effects.test.ts`, `tests/commands/mv.test.ts`, `tests/coverage-hardening/sources-resolution.test.ts`, `tests/source-providers/provider-utils.test.ts`, `tests/source-resolve.test.ts`, `tests/source.test.ts`; integration scope: `tests/integration/indexer.test.ts`, `tests/integration/package-launcher.test.ts`, `tests/integration/ripgrep-install.test.ts`, `tests/integration/walker.test.ts`. Note `tests/commands/mv.test.ts` and `tests/integration/walker.test.ts` carry the symlink-containment coverage for `mv-cli.ts` — the surface Chunk 6 rewrites — so their presence in the boundary record matters downstream. (`tests/file-context.test.ts` is NOT symlink coverage — grep shows only a variable named `realPath`; do not list it.)
  - **§15.3 boundary set = the union: 29 files / 9,844 LOC.** WI-08 replays all 29 so every plan-named category appears in the boundary record Chunks 7/6/9 inherit.
...
3. Targeted safety replay (explicit, for the boundary record): `bun test --timeout=30000` over the **full §15.3 boundary set from §2.5 — the 19 dedicated safety files PLUS the 10 symlink-coverage carrier files (29 files / 9,844 LOC)** — so all five plan-named categories (traversal/escape, symlink, redaction/dangerous-key, SQLite, migration) appear in the boundary record Chunks 7/6/9 inherit. Record the per-category file list in report.md, including the note that symlink coverage is embedded (no dedicated suites) and that `mv.test.ts`/`walker.test.ts` carry the symlink-containment coverage for the `mv-cli.ts` surface Chunk 6 rewrites.
...
- [R9] `check:fast` + `check` green at the boundary; the full 29-file §15.3 boundary set (19 dedicated + 10 symlink carriers — all five plan-named categories) replayed green; outputs recorded.
