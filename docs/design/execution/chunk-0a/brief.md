...
- **Revision 3** (adversarial-panel corrections): the §2.5 §15.3 boundary set is now a **reproducible, basis-stated enumeration** (filename-grep per category + content confirmation, with explicit exclusion rationales) — it adds six grep-discoverable in-category dedicated suites the 19-file list silently omitted (`improve-redaction`, `wiki-ingest-redaction` under redaction/dangerous-key; `improve-lock-invariants`, `improve-db-locking`, `improve-skip-if-locked` under SQLite/lock/contention; `stash-dir-safety` ruled INTO traversal/escape) plus three further migration suites found by the same basis, yielding **28 dedicated files / 5,636 LOC**, union with the 10 symlink carriers = **38 files / 11,240 LOC**, reconciled against the plan's "~22 / ~4,700" in §2.5; WI-08 step 3 and its acceptance replay the revised set. Carried from Revision 2: five-category coverage incl. embedded symlink; both test-LOC denominators stated (plan §15 header 588/175,041 over all `tests/**/*.ts` is exact, not stale); WI-02's P0-A once-per-asset scenario emits the `reflect_invoked` cursor event explicitly (the gate is event-driven — a stubbed `reflectFn` never emits it).
...
3. Commit the re-measured anchor table (brief §2, verbatim — **including §2.5's basis-stated §15.3 boundary-set enumeration, its explicit exclusions, and the reconciliation against the plan's "~22 files / ~4,700 LOC"**) as `docs/design/execution/chunk-0a/anchors.md`, headed by the HEAD sha and the rule "these values supersede plan line numbers for this chunk".
...
- `tests/commands/improve/goldens-p0a-selection.test.ts` — cases:
  ...
  3. once-per-asset: run once **with the `reflect_invoked` cursor event emitted for the selected ref** (see step 3 — the stub bypasses `akmReflect`, so the event must be emitted explicitly; a persisted proposal alone does NOT move the gate), run again → **zero** P0-A selections (gate `!lastReflectProposalTs.has(ref)` `preparation.ts:1327-1330`; map built at `:1068` by `buildLatestProposalTsMap`, which sources EXCLUSIVELY from `reflect_invoked` events);
...
3. **Once-per-asset cursor emission (precondition for P0-A scenario 3)**: the gate reads `buildLatestProposalTsMap` (`eligibility.ts:382-410`), which sources EXCLUSIVELY from `reflect_invoked` events in state.db, keyed by `improveStateReadRefs(ref, sourceName, includeLegacyBare)` durable keys. That event is emitted only at `akmReflect` entry (`reflect.ts:953`) — the recording stub bypasses `akmReflect`, and `createProposal` emits no event, so under a bare stub run 1 leaves the cursor map empty and run 2 RE-SELECTS the ref (the wrong behavior to golden). The stub must therefore emit the cursor event itself per selected ref — mirroring akmReflect: `appendEvent({ eventType: "reflect_invoked", ref: <durable ref>, metadata: { eligibilitySource } })` — or the harness must `appendEvent` one between run 1 and run 2. Use the existing patterns in the referenced harness file: `appendEvent({ eventType: "reflect_invoked", ref: "memory:already" })` at `tests/commands/improve/improve-eligibility.test.ts:754` (inside "P0-A fires at most once per asset", `:748`) and the sourceName-qualified/injected-`now` form at `:178-179` (`appendEvent(..., { now })`, import from `src/core/events`). The ref key must match the `improveStateReadRefs`/`sourceName` resolution or the map will not resolve it: bare `memory:<name>` for a local stash, `<source>//memory:<name>` when `sourceName` is set.
4. Serialize goldens via `expectGolden` to `tests/fixtures/goldens/improve/self-consistency.json` and `.../p0a-selection.json`: counts, draftMode histograms, per-lane counts, fixture-local membership, once-per-asset outcomes. NO production refs, no event-count assertions (the step-3 event is a harness *input*, never an asserted output).
5. Suite comments must state: (a) stubbed `reflectFn` means `reflect_invoked` events are NOT emitted per SC sample — counts come from stub invocations; production telemetry sees 3 `reflect_invoked` per hot ref (`reflect.ts:953`); (b) these goldens are re-baselined in Chunk 7 (designation); (c) **the P0-A once-per-asset gate cursor is EVENT-driven (`reflect_invoked`), not proposal-record-driven** — a persisted reflect proposal without the event does not block re-rescue; Chunk 7's diff review needs this distinction to read the lane deletion correctly. Record note (c) in the p0a fixture's `notes` field as well.
6. Add both fixtures to `DESIGNATIONS.json` as `re-baseline` @ `7`, consumers = the two suites.
...
- [R2] P0-A selection sets pinned: threshold, once-per-asset (via explicit `reflect_invoked` cursor emission per step 3 — zero selections on run 2), per-lane attribution incl. proactive/high-salience isolation run; suite/fixture notes record that the gate cursor is event-driven.
...
3. Targeted safety replay (explicit, for the boundary record): `bun test --timeout=30000` over the **full §15.3 boundary set from §2.5 — the 28 dedicated safety files PLUS the 10 symlink-coverage carrier files (38 files / 11,240 LOC)** — so all five plan-named categories (traversal/escape, symlink, redaction/dangerous-key, SQLite, migration) appear in the boundary record Chunks 7/6/9 inherit. Record in report.md: the per-category file list, the enumeration basis (§2.5 grep patterns + content confirmation), the explicit exclusions with rationale, the reconciliation against the plan's "~22 files / ~4,700 LOC", the note that symlink coverage is embedded (no dedicated suites), and that `mv.test.ts`/`walker.test.ts` carry the symlink-containment coverage for the `mv-cli.ts` surface Chunk 6 rewrites while `improve-redaction.test.ts` and the improve-lock trio cover the improve surface Chunk 7 rewrites.
...
6. Write `docs/design/execution/chunk-0a/report.md`:
   ...
   - **Net-LOC**: `src/` = 0 (verified); test+fixture LOC added (signed, from `git diff --shortstat`), ledgered separately per §15 rule 9. Test-suite baseline at `3d9ee7b`, both denominators verified exact and stated with their basis: **all `tests/**/*.ts` = 588 files / 175,041 LOC — this matches the plan §15 header exactly (the plan figure is correct; do NOT mark it stale)**; `*.test.ts`-only = 564 files / 172,155 LOC (a different, narrower denominator). Ledger this chunk's added test/fixture LOC against the all-`tests/**/*.ts` denominator (the plan §15 basis), labeling the denominator in the report.
...
- [R9] `check:fast` + `check` green at the boundary; the full 38-file §15.3 boundary set (28 dedicated + 10 symlink carriers — all five plan-named categories, per §2.5's basis-stated enumeration) replayed green; outputs recorded incl. exclusion rationales and the plan-count reconciliation.
...
2. **Stub-driven `reflectFn` bypasses `akmReflect`, so NO `reflect_invoked` events are emitted** — two distinct consequences: (a) event-based SC call counts would undercount → count stub invocations, document the production-telemetry note (WI-02 step 5); (b) the P0-A once-per-asset gate (`buildLatestProposalTsMap` reads `reflect_invoked` events ONLY — the cursor is event-driven, not proposal-record-driven) never engages under a bare stub, so a naive two-run scenario re-selects the ref and would golden the WRONG once-per-asset behavior as the R2/§5 oracle → the stub (or harness) emits the cursor event explicitly (WI-02 step 3).
