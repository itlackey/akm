...
- **Scored path** (`searchDatabase`, fn starts :282): `sourceFiltered` :513-515 → `scopeFiltered` :520-522 (`entryMatchesScope`, db-search.ts:927) → `qualityFiltered` :527-529 (`isProposedQuality`, `src/indexer/passes/metadata.ts:245`) → `beliefFiltered` :530 (`matchBeliefFilter`, db-search.ts:692) → `selected = beliefFiltered.slice(0, limit)` :534. ALSO calls `inheritDerivedTwinBeliefStates(db, scored)` at :459 (pre-ranking).
- **Enumerate path** (`enumerateEntries`, fn starts :574): `sourceFiltered` :619-621 → `scopeFiltered` :625-627 → `qualityFiltered` :628-632 → `inheritDerivedTwinBeliefStates` :636 → `beliefFiltered` :637 → `selected` :638.

**CORRECTION (WI-0b.5, empirically verified — supersedes this section's original claim):** the census originally asserted the scored path "lacks the `inheritDerivedTwinBeliefStates` call, a documented asymmetry." That is WRONG — `inheritDerivedTwinBeliefStates` is called on BOTH paths (scored :459, enumerate :636), and the scored path's belief filter derives from the same mutated array (chained `.filter()` preserving object references), so both paths' belief filters see inherited twin state identically (proven by the `base-memory`/`base-memory.derived` parity case in the WI-0b.5 golden). The REAL two-path asymmetry is CANDIDATE-POOL CONSTRUCTION: the scored path's pool is `searchFts`/vector matches for the query's own tokens (FTS indexes description/tags/searchHints/aliases — NOT body prose), so a `.derived` twin sharing no indexed token with the query is NEVER a scored candidate under any belief filter; the enumerate path's pool is `getAllEntries` (query-independent), so the twin always appears and inherits state (proven by the `silent-twin-base.derived` divergence case). This is what the WI-0b.5 `filter-behavior/scored-vs-enumerate.json` golden pins for Chunk 5 (and what Chunk 5 must preserve or deliberately re-baseline).

A 0b filter-behavior golden captures BOTH paths' output sets (proposed/belief/scope combinations) since they are not yet unified — the candidate-pool divergence above is the divergence the golden pins before Chunk 5 unifies them.
