...
| #632 | FIXED | recombine clusters = whole-stash tag buckets → bland hypotheses. (a) Graph-entity clustering now the DEFAULT (`relatednessSource` "tags"→"both") + `isJunkEntity`/`excludeEntities` hygiene; (b) ROOT FIX — exclude session-capture telemetry memories (`<harness>-(session\|checkpoint)-<YYYYMMDD>-<id>`, `akm_memory_kind: session_checkpoint`) from the pool: ~20% of memories, their embedded metadata block was the source of every top noise entity (`session_checkpoint`/`harness`/`buffered observations`). After exclusion the top real-data clusters are coherent subjects (print-md, openpalm, guardian, paged.js, databasin) not session bookkeeping. Additive: tag streaks untouched (`entity:` namespace), tag fallback covers memories with no graph entity. |
