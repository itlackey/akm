...
5. **Goldens re-baselined once, deliberately — and mechanically present.** Enumerate the 35+ golden/characterization assets (CLI output baselines, the ranking-baseline fixture stash, SQLite-migration snapshots, 6 characterization suites); each is designated (a) frozen as migration-input fixture or (b) re-captured post-cutover in its designated chunk with a reviewed diff. Re-recording outside the designated chunk is forbidden. **Presence is a lint, not a promise** (gate hardening, 2026-07-16, after the 07-15 purge deleted the entire Chunk 0a oracle set and the chunk gates kept passing vacuously): `scripts/lint-goldens-presence.ts` (in `bun run lint`) fails when `DESIGNATIONS.json` or any registered asset/consumer suite is missing, so every chunk baseline, Finalize, and CI run enforces the oracles' existence. A gate that references goldens is RED — never skipped — when the referenced files are absent. The 0a set (49 surviving designations; SC/P0A retired by chunk 7's ledger) was restored from git and re-validated green at the chunk-7 HEAD.
