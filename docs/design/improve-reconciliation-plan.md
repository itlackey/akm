...
| WS-3b intake+homeostatic (#604) | done | 79ffc63a | pass / confirmed-green | none | Gated hot-probation filter in consolidate.ts behind `processes.extract.hotProbation.enabled` (same config path extract.ts uses); was unconditional on every call; now default-off, matching all other WS-3b subsystems. I/O win + correctness-preserving (no asset can carry the marker when flag is off). |
