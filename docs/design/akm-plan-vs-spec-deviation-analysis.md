...
> **Status (2026-07-14, closed):** the §4 resolutions below — including the 2026-07-13/14 refinements 3a (bindings → Tier A), 3b (memory lifecycle → deferred entirely), and 3c (new trust/approval machinery → dropped) — are **applied in place** across all documents: the normative spec is amended to v0.3 (reconciled ref grammar, open `type`, optional-method adapter contract, release-staging notes at §18/§25), the bundle-adapter spec and the 0.9.0 plan encode the final scope in their operational sections (two-wave chunk sequence, reported ledger, DoD, contract tests), and the decision history carries D27–D30. This document is retained as the decision record; it no longer describes open deviations.
...
3. **DEV-5 — RESTORE (naming).** The third `consolidate` verb is adopted as vocabulary. Retained simplifications: renderer/action as a data table, and adapter facets as **optional methods** on one interface (not a rigid hierarchy — History §8.3). The storage `Repository<Row,Domain>` decision is unrelated and stands (ship `jsonColumn()` only).
3a. **DEV-3 Bindings — REVISED to Tier A (2026-07-13 refinement, supersedes the earlier "restore full").** Code ground-truth shows the behaviors the binding concept names already exist: install≠activation is already true (`akm add`/`source-add.ts` only indexes; scheduling is an explicit `akm tasks add`/`enable` → `sched.install`); engines/connections are already workspace-config-owned with credentials never in bundles (symbolic `$VAR`, materialized at dispatch — `engine-resolution.ts:110-237`); runtime ref→values resolution already exists (`resolveEnvBinding`, `${secret:NAME}`); first/third-party policy is already enforced (`registryId` block/warn + add-time dangerous-key scan). The only genuinely-new machinery is a *persisted* approval/enable/trust record carrying an export digest + rebind-on-update — for ~one consumer today. **Tier A ships in 0.9.0:** binding = the terminology + consolidation of those existing behaviors into one workspace activation-policy point, plus the untrusted read-path clamp (already in Chunk 2/5). **Tier B defers** (until a real untrusted-bundle-ships-a-runnable-export consumer exists): the persisted `workspace_bindings` record, export digests, rebind-on-update (normative §18.5), and the `akm bind|unbind|bindings` CLI. Deferring Tier B loses no current safety property. **env/secret handling is UNCHANGED** — they remain whole-file assets inside stashes/bundles (`<stash>/env/`, `<stash>/secrets/`) with the existing `registryId` policy; nothing moves workspace-side. (An earlier note suggesting env/secret move workspace-side was a misread and is dropped.)
3b. **DEV-4 Memory lifecycle — REVISED to DEFERRED ENTIRELY (2026-07-14 decision, supersedes the earlier "restore full").** The external code-quality review identified the §25 state model (+2,500–3,000 LOC: operational states, grace/restore, CAS archive, sandbox replay, water-marks) as feature work in a refactor's clothing whose central gate (claim coverage) depends on an extractor that does not exist. **0.9.0 ships only the consolidate.ts decomposition with existing behavior preserved exactly** (plan §6): the current merge/delete/promote/contradict ops, `archiveMemory`, journals, LOOK/CHANGE separation, hot-capture guard, proposal-gating — all as today, verified by goldens. The entire lifecycle waits for 0.9.1+ and starts only when the claim extractor + its benchmark exist; normative §25 is target-state, not release scope.

3c. **Trust/approval machinery — DROPPED for 0.9.0 (2026-07-14 decision).** All *new* trust/security machinery is rejected, not just deferred-by-tier: no trusted labeling in search/show, no action-string clamping, no confirm prompts, no generic-files sensitive-content refusal, no digests, no trust records. Rationale (maintainer): these are nice ideas that in practice are not helpful, provide a false sense of security, and force maintenance of brittle code. Only protections that exist in code today survive the port unchanged (env/secret redaction renderers, the `registryId` dangerous-key block/warn, the add-time dangerous-key scan), with port-preservation tests. The known residual (workflow refs resolving into installed sources re-read current disk content per invocation) is accepted by design — crontab semantics; operators choose what they install and reference (plan Chunk 6.5).

4. **DEV-6 Facets** — resolved by #3: capabilities as optional methods on one interface.
...
Applied to: the reconciled `akm-0.9.0-bundle-adapter-spec.md` (banner + §10/§12), the normative spec (v0.3: §18/§25 staging notes; trust-clamp text removed from §15/§22/§28/§31/§32), the plan in full (banner, §1, §2, §3.2, §5, §6, §8, the two-wave §11, §12, §13, §15, §16), and the decision history (D8 note, D27–D30, §5.4/§5.5). The 2026-07-13 Tier-A binding refinement (#3a), the 2026-07-14 memory deferral (#3b) and trust-machinery drop (#3c), the reported-ledger and hygiene-first-sequencing decisions, and the offline metrics-harness refinement (plan §13.2, incl. the AKM_EMBED_DETERMINISTIC facade carve-out and the one-shot `asset_outcome` exporter) are all applied in place.
