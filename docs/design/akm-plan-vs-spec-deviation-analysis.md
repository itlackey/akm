...
3. **DEV-4/5 — RESTORE.** The full bounded memory lifecycle (normative §25) and the third `consolidate` verb are in scope for 0.9.0. Retained simplifications: renderer/action as a data table, and adapter facets as **optional methods** on one interface (not a rigid hierarchy — History §8.3). The storage `Repository<Row,Domain>` decision is unrelated and stands (ship `jsonColumn()` only).
...
Applied to: the reconciled `akm-0.9.0-bundle-adapter-spec.md`, and the plan's reconciliation banner + §5/§6/§13.3/Chunk-4 edits. The 2026-07-13 Tier-A binding refinement (#3a) is applied in place in the plan at Chunk 6.5, §12.1 (ledger), §12.2 (DoD 10), §12.3, §12.4, and §13.3; the offline sandbox metrics-harness refinement is applied at §13.2.
