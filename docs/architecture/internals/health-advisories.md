...
| `txn-awaiting-recovery` | `$DATA/txn/<rootNs>/` holds a trusted, fenced transaction journal older than one hour whose `rollback`/`finalize` failed and was deferred rather than quarantined (`akm migrate apply` retries these on its own). | For a named proposal, run `akm proposal show <id>` then accept, reject, or revert it; `akm migrate status` lists every stuck journal. |
