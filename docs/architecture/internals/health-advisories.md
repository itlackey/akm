...
| `txn-quarantine` | `$DATA/txn-quarantine` holds at least one transaction journal `akm migrate apply` could not trust (an unreadable `journal.json` or a fence violation) and quarantined instead of aborting recovery. | Inspect the named journal's `reason.json`; nothing was deleted, so it can be restored by hand once the cause is understood. |
| `txn-awaiting-recovery` | `$DATA/txn/<rootNs>/` holds a trusted, fenced transaction journal older than one hour whose `rollback`/`finalize` failed and was deferred rather than quarantined (`akm migrate apply` retries those under the default stash). | For a named proposal, run `akm proposal show <id>` then accept, reject, or revert it; `akm migrate status` lists those under the default stash. |
...
| `version-reconcile` | (upgrade-D/D2) Reads the host-local reconciliation stamp (`$STATE/version-reconcile.json`) `akm-migrate apply --host-local` writes on every CLI startup. `pass` only when the stamp's version matches the running CLI; `unknown` when the stamp exists but can't be parsed. Never spawns anything itself. | A `warn` means this host has not reconciled config/state.db/scheduler grants to the version that's actually running — a stale stamp, a missing one, or a blocked last attempt (the blocker is named in the message). Run `akm migrate status` for detail. |
| `scheduler-grants` | (upgrade-D/D2) Installed native scheduler rows (the operator's own prior `akm task sync`) that have a backing asset file but no host-local grant yet (`pendingGrantsFromInstalled`). `--probe`-gated like `scheduler-binary`: `unknown` "not probed" with `--no-probe`. `unknown` when the backend can't be selected or can't inspect its own bindings. | A `warn` names the ungranted refs — run `akm task sync` to carry them forward. |
| `scheduled-startup-failures` | (upgrade-D/D2) `task_history` rows in the health window that failed with a startup-class exit code (2 usage, 70 internal, 78 config) — a run that died before its body executed. `unknown` when `task_history` can't be queried. | A `warn` names the failing task id(s) and, when a log is on disk, the first line naming the error. This is the signature of a stale scheduled binary or a broken config/migration, not an ordinary task-body failure — check `akm migrate status` and the scheduled binary (`scheduler-binary` above). |
| `akm-installs` | (upgrade-D D3, upgrade-D3 r3-4) Enumerates every `akm` executable on PATH and in the known install roots (bun global, the npm global root, pnpm global, `~/.local/bin`, `/usr/local/bin`, every nvm node version's `bin/`), deduped by realpath, and probes each with `--version` — the same enumeration `akm upgrade` uses to move every install it can. It also derives the npm global root of every distinct `node` executable found on PATH, in an nvm `bin/` dir, or in `~/.bun/bin` (not just the running node's), scanning each root's own `akm-cli/dist` directly so a copy `npm install -g`'d there is reported even when it was never linked onto PATH, and scans `${BUN_INSTALL:-~/.bun}/lib/node_modules` unconditionally — the layout a plain `npm install -g` produces when it runs under bun's `node -> bun` shim, distinct from bun's own global install. `--probe`-gated: `unknown` "not probed" with `--no-probe`. `unknown` when no install could be found anywhere, or every found install's `--version` probe failed. | A `warn` lists each install (by path) whose version differs from the running one, with the manager command that pins it to the running version (`npm install -g akm-cli@<running version>`, etc., via the same `getPackageManagerUpgradeCommand` `akm upgrade` uses — never `@latest`, so a prerelease host isn't told to downgrade); when that install has its own adjacent npm/pnpm in a `binDir`, the command is shown prefixed with `PATH="<binDir>:$PATH"` (r3-2) so pasting it runs under that install's own `node`, not the one already on PATH; an install with no attributable manager names "reinstall it manually". An npm install the direct `akm-cli/dist` scan found but that is not linked onto any bin dir is instead marked "not on PATH", and its remedy is to remove that package directory — no package manager command can ever update it. On Windows, npm's link is a cmd-shim file in its own prefix (`%APPDATA%\npm\akm.cmd`), so a prefix holding `akm.cmd` counts as linked and is reported as that install's bin directory; the remedy is then shown as `<binDir>\npm.cmd install -g akm-cli@<running version>`. Run `akm upgrade`, which moves every install it can on its own. |
| `plugin-version` / `opencode-plugin-version` | (itlackey/akm#832, upgrade-D D3) `plugin-version` reports each installed Claude Code harness plugin's version and its declared `AKM_VERSION_RANGE` admission of the running CLI. `opencode-plugin-version` does the same for the OpenCode plugin's own bundled `akm-cli` (read from its `package.json`, never executed) — it runs in-process and shares the host's databases, so its lag matters the same way. Neither is `--probe`-gated. | A `warn` names the stale/out-of-range plugin and the update command (`claude plugin update <ref>` for the Claude plugin; update the OpenCode plugin for the bundled copy). |
