...
| `txn-quarantine` | `$DATA/txn-quarantine` holds at least one transaction journal `akm migrate apply` could not recover (a fence violation or a `rollback`/`finalize` throw) and quarantined instead of aborting recovery. | Inspect the named journal's `reason.json`; nothing was deleted, so it can be restored by hand once the cause is understood. |
