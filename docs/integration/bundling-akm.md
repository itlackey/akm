...
| `deadResidue` / `staleTxns` | Present only when a bundle is configured. `{ pending: [...] }` under a read-only run, `{ removed: [...] }` / `{ recovered: [...], quarantined: [...] }` after apply. A journal `recoverTxnsForRoot` could not recover (fence violation or handler throw) is quarantined to `$DATA/txn-quarantine`, not thrown — it does not set `status` to `blocked`. |
