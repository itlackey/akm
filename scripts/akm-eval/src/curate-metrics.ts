 ...
 * and the in-process CI guard (`tests/curate-golden-eval.test.ts`). No IO,
 * no akm imports — just arrays of refs in returned order vs. labeled
 * judgments.
 */
...
export function noBannedAboveRequired(
  ...
  const relevantRanks = returned
    .map((ref, i) => (relevant.has(ref) ? i : -1))
    .filter((i) => i >= 0);
...
export function scoreCurateCase(
  ...
  const score =
    ...
    rr * weights.mrr +
    gate.score * weights.noBannedAboveRequired;
  return {
    ndcg,
    recall,
    mrr: rr,
