...
- **The memory precheck's content read (and the ref it fed into promotion classification) could target a different file than distill's real dispatch.** `runLoopDistillPass`'s memory branch resolved both the asset-content lookup and the `wouldPromoteMemoryToKnowledge` `durableInputRef` field via `planned.itemRef ?? planned.ref`, but `akmDistill`'s real dispatch always derives `durableInputRef` — and reads/scores content — from `options.ref` alone, never `options.itemRef` (`loadAndScoreInputSalience`'s `lookup(durableInputRef)`). The precheck now computes a separate ref for the content lookup and the promotion classification's `durableInputRef` field from `planned.ref` alone, matching distill's real dispatch; the `itemRef`-preferring key stays only for the feedback-events query, which does match `readDistillFeedback`'s `ref: options.itemRef ?? durableInputRef`.
