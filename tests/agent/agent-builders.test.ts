...
// ── config.ts — commandBuilder / modelAliases parsing ────────────────────────

describe("parseAgentConfig — commandBuilder field", () => {
  test("valid commandBuilder string is parsed correctly", async () => {
    const { parseAgentConfig } = await import("../../src/integrations/agent/config");
    const parsed = parseAgentConfig({
      profiles: {
        myprofile: { bin: "my-cli", commandBuilder: "claude" },
      },
    });
    expect(parsed?.profiles?.myprofile?.commandBuilder).toBe("claude");
    expect(warnings).toHaveLength(0);
  });

  test("invalid commandBuilder (number) is warn-and-ignored", async () => {
    const { parseAgentConfig } = await import("../../src/integrations/agent/config");
    mockedQuiet = true;
    const parsed = parseAgentConfig({
      profiles: {
        myprofile: { bin: "my-cli", commandBuilder: 123 },
      },
    });
    expect(parsed?.profiles?.myprofile?.commandBuilder).toBeUndefined();
    expect(warnings.some((w) => w.includes("commandBuilder"))).toBe(true);
  });
});

describe("parseAgentConfig — modelAliases field", () => {
  test("valid modelAliases object is parsed with keys lowercased", async () => {
    const { parseAgentConfig } = await import("../../src/integrations/agent/config");
    const parsed = parseAgentConfig({
      profiles: {
        myprofile: {
          bin: "my-cli",
          modelAliases: { Fast: "opencode/claude-haiku-4-5", TURBO: "opencode/claude-sonnet-4-6" },
        },
      },
    });
  ...
  test("invalid modelAliases (string) is warn-and-ignored", async () => {
    ...
    const parsed = parseAgentConfig({
      profiles: {
        myprofile: { bin: "my-cli", modelAliases: "fast=opencode/claude-haiku-4-5" },
      },
    });
    expect(parsed?.profiles?.myprofile?.modelAliases).toBeUndefined();
    expect(warnings.some((w) => w.includes("modelAliases"))).toBe(true);
  });

  test("modelAliases values that are not strings are skipped silently", async () => {
    const { parseAgentConfig } = await import("../../src/integrations/agent/config");
    mockedQuiet = true;
    const parsed = parseAgentConfig({
      profiles: {
        myprofile: {
          bin: "my-cli",
          modelAliases: {
            fast: "opencode/claude-haiku-4-5", // valid
            broken: 42, // invalid — non-string value
            alsobad: null, // invalid
          },
        },
      },
    });
...
// ── config.ts — resolveAgentProfile modelAliases merge ───────────────────────

describe("resolveAgentProfile — modelAliases merge", () => {
  test("user modelAliases merged on top of base profile; user wins per key", async () => {
    const { resolveAgentProfile } = await import("../../src/integrations/agent/config");
    // "opencode" built-in has no modelAliases — inject via overrides.
    // Simulate a base with existing aliases by using a custom profile name
    // and providing both base-like and override aliases.
  ...
  test("user modelAliases merged on top of hypothetical base aliases via custom profile", async () => {
    const { resolveAgentProfile } = await import("../../src/integrations/agent/config");
    // Use a custom (non-built-in) profile name so we control the full shape.
    // This verifies the spread merge logic: { ...base.modelAliases, ...overrides.modelAliases }
    const profile = resolveAgentProfile("my-agent", {
      bin: "my-cli",
  ...
  test("when no user modelAliases provided, base modelAliases is retained as-is", async () => {
    const { resolveAgentProfile } = await import("../../src/integrations/agent/config");
    // Override everything except modelAliases — base should flow through unchanged.
    const merged = resolveAgentProfile("opencode", { stdio: "captured" });
    // opencode builtin has no modelAliases, so result should be undefined or empty.
    // The important check is that no error is thrown and the profile resolves.
    expect(merged).toBeDefined();
    expect(merged?.stdio).toBe("captured");
  });

  test("user modelAliases key overwrites base key with same name", async () => {
    const { resolveAgentProfile } = await import("../../src/integrations/agent/config");
    // Create a profile from a fake base that has modelAliases, then override one key.
    // Since built-ins don't have modelAliases, we use a two-step approach:
    // first resolve with a base alias, then resolve again with override.
    // Directly test the merge contract using a custom profile.
    const first = resolveAgentProfile("my-agent", {
      bin: "my-cli",
