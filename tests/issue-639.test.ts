...
import { classifyProposal, type DrainPolicy } from "../src/commands/proposal/drain";
...
describe("#639 Part B — minConfidence in DrainAcceptRule / classifyProposal", () => {
  // A policy that requires confidence >= 0.8 for reflect proposals.
  const POLICY_WITH_MIN_CONFIDENCE: DrainPolicy = {
    name: "test-min-confidence",
    accept: [
      {
        generator: "reflect",
        maxDiffLines: 80,
  ...
  function makeProposal(confidence?: number): Proposal {
    return {
      id: `prop-${Math.random().toString(36).slice(2)}`,
      ref: "lesson:test-confidence",
      source: "reflect",
      status: "pending",
      sourceRun: "run-639",
  ...
  test("B1: proposal with confidence < minConfidence is DEFERRED", () => {
    // confidence=0.5 < minConfidence=0.8 => deferred.
    // FAILS in RED state: classifyProposal does not check minConfidence yet
    // and returns { verdict: 'accept' } for all in-bounds reflect proposals.
    const proposal = makeProposal(0.5);
    const decision = classifyProposal(proposal, POLICY_WITH_MIN_CONFIDENCE);
    expect(decision?.verdict).toBe("defer");
  });

  test("B2: proposal with confidence === minConfidence is ACCEPTED", () => {
    // confidence=0.8 === minConfidence=0.8 => accepted (floor is inclusive).
    // FAILS in RED state until minConfidence is honored, but the accept verdict
    // itself would still be correct — verifies the boundary is inclusive.
    const proposal = makeProposal(0.8);
    const decision = classifyProposal(proposal, POLICY_WITH_MIN_CONFIDENCE);
    expect(decision?.verdict).toBe("accept");
  });

  test("B3: proposal with confidence > minConfidence is ACCEPTED", () => {
    // confidence=0.95 > minConfidence=0.8 => accepted.
    const proposal = makeProposal(0.95);
    const decision = classifyProposal(proposal, POLICY_WITH_MIN_CONFIDENCE);
    expect(decision?.verdict).toBe("accept");
  });

  test("B4: proposal with NO confidence field PASSES (absent confidence = PASS)", () => {
    // Absence of confidence means the source did not self-estimate quality.
    // Per the ticket: "Absent confidence => PASS". This is a default-preserving
    // contract — must be GREEN.
    const proposal = makeProposal(undefined);
    const decision = classifyProposal(proposal, POLICY_WITH_MIN_CONFIDENCE);
    // Absent confidence must not defer — should be accept (rule matches, no floor hit).
    expect(decision?.verdict).toBe("accept");
  });

  test("B5: PERSONAL_STASH default has NO minConfidence (default-preserving guard)", () => {
    // PERSONAL_STASH should not change — no minConfidence default is added.
    // This is a permanent regression guard: the built-in preset stays intact.
    for (const rule of PERSONAL_STASH.accept) {
      expect(rule.minConfidence).toBeUndefined();
    }
  });

  test("B6: classifyProposal WITHOUT minConfidence policy is byte-identical to pre-#639", () => {
    // Using PERSONAL_STASH (which has no minConfidence): a reflect proposal
    // with confidence=0.3 must still be accepted (pre-#639 behavior). The new
    // minConfidence gate only fires when the RULE defines it.
    const proposal = makeProposal(0.3);
    const decision = classifyProposal(proposal, PERSONAL_STASH);
    // The reflect rule in PERSONAL_STASH has maxDiffLines:80; VALID_LESSON is
    // small enough to pass. No minConfidence => must accept.
    expect(decision?.verdict).toBe("accept");
  });

  test("B7: DrainAcceptRuleSchema in drain-policies.ts accepts minConfidence field", async () => {
    // The custom-policy file schema (DrainAcceptRuleSchema) must accept minConfidence.
    // This test dynamically resolves the policy file so failures are semantic, not
    // import-time. We test this via resolveDrainPolicy with a tmp JSON file.
    //
    // FAILS in RED state: DrainAcceptRuleSchema uses .strict() and rejects the
    // unknown 'minConfidence' key.
    const fs = await import("node:fs");
    const os = await import("node:os");
    const path = await import("node:path");
    const { resolveDrainPolicy } = await import("../src/commands/proposal/drain-policies");

    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "akm-639-policy-"));
    const policyPath = path.join(tmpDir, "policy.json");
    try {
      const policy = {
        name: "test-639",
        accept: [{ generator: "reflect", maxDiffLines: 80, minConfidence: 0.8 }],
        rejectEmpty: true,
        defer: [],
      };
      fs.writeFileSync(policyPath, JSON.stringify(policy));
      // resolveDrainPolicy should NOT throw a UsageError for minConfidence.
      const resolved = resolveDrainPolicy(policyPath);
      expect(resolved.name).toBe("test-639");
      const rule = resolved.accept[0];
      expect(rule?.minConfidence).toBe(0.8);
    } finally {
