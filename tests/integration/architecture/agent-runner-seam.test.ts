...
describe("RunnerSpec dispatch authority", () => {
  ...
  test("low-level runner reference guard catches aliased imports", () => {
    const aliasedImports = [
      `import { runAgent as invoke } from "../../architecture/spawn";`,
      `import { runOpencodeSdk as invokeSdk } from "../../architecture/sdk-runner";`,
      `const { runAgent: dynamicAlias } = await import("../../architecture/spawn");`,
    ].join("\n");
