...
import { ensureUnitTextTables } from "../../../src/storage/repositories/unit-texts-repository";
import { ensureUnitTables, searchUnits, type UnitVecHit } from "../../../src/storage/repositories/units-repository";
import {
  type Cleanup,
  sandboxStashDir,
  sandboxXdgCacheHome,
...
function openSeededDb(label: string): Database {
  const db = openIndexDatabase(tmpDbPath(label));
  ensureUnitTextTables(db);
...
// ── searchUnits (A2 stub) — identity filtering sanity check ────────────────

describe("searchUnits (stage-1 stub) identity filtering", () => {
...
describe("fuseByEntry — reciprocal rank fusion", () => {
  ...
  test("semantic-only hit: rankingMode 'semantic', score = 1/(RRF_K + rank)", () => {
    ...
    try {
      ...
      const semanticHits: UnitVecHit[] = [{ unitId: 1, hash: "hS", distance: 0.05 }];
  ...
  test("both lists hit different units of the SAME entry: hybrid, grouping keeps the better-ranked unit", () => {
    ...
    try {
      ...
      const semantic: UnitVecHit[] = [{ unitId: 1, hash: "hA-frag", distance: 0.02 }];
