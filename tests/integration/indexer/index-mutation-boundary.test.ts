...
// its FK-linked `utility_scores`) survive with the same id, only its unit
// projection (content-addressed, B5h retired the legacy entry-keyed vector
// cache this test used to also seed and re-check here) is refreshed to the
// new content.
...
for (const scenario of [
  ...
  test(`a rename in the ${scenario.label} re-points the same row and publishes one generation`, async () => {
    ...
    // everything keyed off that id — the `utility_scores` row seeded above
    // included — survives with it.
