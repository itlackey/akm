...
bun run tests/bench/cli.ts tests/bench/configs/nano-quick.json # Quick agent bench smoke test (~10 min)
bun run tests/bench/cli.ts tests/bench/configs/full.json       # Full 40-task agent bench (~2-3 hours)
bun run tests/bench/doctor.ts                                   # Bench harness health check
```

Run nano-quick after changes to `src/output/`, `src/commands/show.ts`, or fixture stashes. Set `BENCH_OPENCODE_MODEL` to the model identifier before running. See `tests/bench/BENCH.md` for full bench documentation. Fixture stash content must teach syntax/schema (HOW), not task-specific answer values (WHAT).
