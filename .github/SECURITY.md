...
- API keys for remote sources should be stored via environment variable references (e.g., `${OPENVIKING_API_KEY}`) in the akm config file, not as plaintext values. The CLI performs environment variable substitution at runtime so that secrets are never persisted in the configuration.
